...
import 'package:homebase/prelaunch.dart';
...
List<Org> orgs = [
  Org(
      ...
      name: "Autonet",
      token: tokens[0],
      description: "Decentralized training and inference for AI models."),
  Org(
      address: generateContractAddress(),
      name: "Quantum Church",
      token: tokens[1],
      description: "In quarks we trust"),
  Org(
      address: generateContractAddress(),
      name: "Lots of Trees",
      token: tokens[2],
      description:
          "Raising funds for planting more of these woody things into the grownd and possibly making things better for all lifeforms."),
  Org(
      address: generateContractAddress(),
      name: "Dem Gyals",
      token: tokens[3],
      description: "We out here now."),
  Org(
      address: generateContractAddress(),
      name: "New World Order",
      token: tokens[4],
      description:
          "If not us, who? The dutch? Not likely. They barely reach 500m with their digging equipment."),
];
...
void main() {
  ...
  List<int> lastActiveHours =
      List.generate(50, (index) => Random().nextInt(200) + 1);
  ...
  for (int i = 0; i < 50; i++) {
    ...
    for (int j = 0; j < numBalances; j++) {
      balances[tokens[Random().nextInt(30)]] =
          (Random().nextInt(1000000) + 100).toDouble();
    }

    DateTime lastActive =
        DateTime.now().subtract(Duration(hours: lastActiveHours[i]));

    humans.add(Human(
        address: address, balances: balances, lastActive: lastActive, pic: ''));
  }
