...
class _ExplorerScreenState extends State<ExplorerScreen> {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          context
              ...
              .selectNetworkByName(widget.networkName!);
        }
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isChainSupported = !auth.isConnected ||
        (auth.chainId != null &&
            networkProvider.isChainSupported(auth.chainId!));
                            ...
                            : Consumer<DaoProvider>(
                                builder: (context, daoProvider, child) {
                                  return AnimatedSwitcher(
                                    ...
                                    child:
                                        _buildDaoContent(context, daoProvider),
                                  );
                                },
                              ),
    ...
    switch (daoProvider.state) {
      ...
      case DataState.loaded:
        ...
        return LayoutBuilder(
            key: ValueKey(
                'loaded_${daoProvider.totalDaoCount}'), // Key changes when data reloads
                  ...
                  return InkWell(
                    ...
                    onTap: () {
                      final selectedNetwork =
                          context.read<NetworkProvider>().selectedNetwork;
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text("Network Not Supported",
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // THE FIX: Get the NetworkProvider to access the selected network.
    final networkProvider = context.read<NetworkProvider>();
    final isMobile = MediaQuery.of(context).size.width < 700;
    final searchBar = TextField(
        onChanged: (value) => daoProvider.search(value),
        decoration: InputDecoration(
            hintText: 'Find DAO by name or address',
    ...
    final daoCount = Text('${daoProvider.totalDaoCount} DAOs',
        style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold));
    // THE FIX: Updated the onPressed handler to navigate to the creator screen.
    final createDaoButton = ElevatedButton(
        onPressed: () {
          if (networkProvider.selectedNetwork != null) {
            context.go('/${networkProvider.selectedNetwork!.name}/create');
          }
        },
    ...
    if (isMobile) {
      return Column(children: [
        ...
        Row(
            ...
            children: [daoCount, createDaoButton])
      ]);
    } else {
      return Row(children: [
      ...
      child: SizedBox(
          ...
          child:
            ...
            IconButton(
                ...
                onPressed:
                    currentPage > 1 ? () => daoProvider.changePage(1) : null),
            IconButton(
                icon: const Icon(Icons.chevron_left),
                onPressed: currentPage > 1
                    ? () => daoProvider.changePage(currentPage - 1)
                    : null),
            Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16.0),
            ...
            IconButton(
                ...
                onPressed: currentPage < totalPages
                    ? () => daoProvider.changePage(totalPages)
                    : null)
          ])),
    );
  }
